...
export async function PATCH(request:Request){
  try{
    ...
    if(b.preference!==undefined){const known=b.preference==='none'||await db.prepare('SELECT id FROM menu_options WHERE id = ?').bind(b.preference).first();if(!known)return Response.json({error:'העדפה לא תקינה'},{status:400});await db.prepare('UPDATE reservations SET preference = ? WHERE id = ?').bind(b.preference,b.id).run();}
