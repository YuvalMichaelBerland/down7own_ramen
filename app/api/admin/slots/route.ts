...
type ReservationRow={id:string;guest_name:string;guest_email:string;party_size:number;starts_at:string};
...
export async function GET(request:Request){
  try{
    ...
    const [slotResult,completionResult,reservationResult]=await Promise.all([
      ...
      db.prepare(`SELECT r.id, r.guest_name, r.guest_email, r.party_size, s.starts_at FROM reservations r JOIN slots s ON s.id = r.slot_id WHERE r.status = 'confirmed' ORDER BY s.starts_at ASC`).all<ReservationRow>(),
    ]);
    const completions=new Map(completionResult.results.map(c=>[c.day_key,c]));
    const grouped=new Map<string,{dayKey:string;startsAt:string;endsAt:string;capacity:number;reserved:number;slotCount:number;slotIds:string[];isOpen:boolean;actualAttendees?:number;completedAt?:string;reservations:{id:string;guestName:string;guestEmail:string;partySize:number}[]}>();
    ...
    for(const r of reservationResult.results){const day=grouped.get(dayKey(r.starts_at));if(day)day.reservations.push({id:r.id,guestName:r.guest_name,guestEmail:r.guest_email,partySize:r.party_size});}
