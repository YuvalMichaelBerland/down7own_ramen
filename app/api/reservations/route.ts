...
type ReservationRow = { id:string; slot_id:string; party_size:number; status:string; starts_at:string; ends_at:string; notes:string };
...
export async function GET(request: Request) {
  try {
    ...
    const rows = await db.prepare(`SELECT r.id, r.slot_id, r.party_size, r.status, r.notes, s.starts_at, s.ends_at FROM reservations r JOIN slots s ON s.id = r.slot_id WHERE r.google_subject = ? AND r.status = 'confirmed' ORDER BY s.starts_at ASC`).bind(user.sub).all<ReservationRow>();
    ...
    return Response.json({ reservations: rows.results.map((r) => ({ id:r.id, startsAt:r.starts_at, endsAt:r.ends_at, partySize:r.party_size, notes:r.notes, preferences: prefsByReservation.get(r.id) ?? Array(r.party_size).fill('none') })) });
...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as { slotId?:string; partySize?:number; preferences?:string[]; notes?:string };
    ...
    try { await db.prepare(`INSERT INTO reservations (id, slot_id, google_subject, guest_name, guest_email, party_size, notes, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'confirmed', ?)`).bind(id, body.slotId, user.sub, user.name, user.email, body.partySize, notes, new Date().toISOString()).run(); }
...
export async function PATCH(request: Request) {
  try {
    ...
    const user = await verifySession(token);
    const body = await request.json() as { id?:string; partySize?:number; preferences?:string[]; notes?:string };
    ...
    await ensureSchema(); const db = database();
    const reservation = await db.prepare(`SELECT r.id, r.slot_id, r.party_size, r.status, s.ends_at, s.capacity FROM reservations r JOIN slots s ON s.id = r.slot_id WHERE r.id = ? AND r.google_subject = ?`).bind(body.id, user.sub).first<{ id:string; slot_id:string; party_size:number; status:string; ends_at:string; capacity:number }>();
