...
type MyReservation={id:string;startsAt:string;endsAt:string;partySize:number};
...
export default function Home(){
  ...
  const loadMine=useCallback(async()=>{if(!credential)return;const r=await fetch('/api/reservations',{headers:authHeaders()});if(r.ok){const d=await r.json() as {reservations:MyReservation[]};setMine(d.reservations||[]);}},[credential,authHeaders]);
  useEffect(()=>{loadMine()},[loadMine]);
  async function cancelMine(id:string){if(!window.confirm('לבטל את ההזמנה?'))return;const r=await fetch('/api/reservations',{method:'DELETE',headers:{'content-type':'application/json',...authHeaders()},body:JSON.stringify({id})});if(r.ok){await loadMine();await load();}}
  async function editMine(id:string,newPartySize:number){const r=await fetch('/api/reservations',{method:'PATCH',headers:{'content-type':'application/json',...authHeaders()},body:JSON.stringify({id,partySize:newPartySize})});const d=await r.json() as {error?:string};if(!r.ok){setError(d.error||'לא הצלחנו לעדכן את ההזמנה');return;}await loadMine();await load();}
  ...
  return <main>
    <nav className="nav"><a className="brand" href="#top"><img src="/ramen-logo.png" alt="Down7own Ramen"/><span>DOWN7OWN RAMEN</span></a>{!credential&&<div className="google-button" style={{marginTop:0}}><GoogleSignIn onCredential={setCredential}/></div>}</nav>
    {credential&&<aside className="mine-panel"><p className="kicker">ההזמנות שלי</p>{mine.length?mine.map(m=><div key={m.id} className="mine-row"><div><strong>{fmtDate(m.startsAt)}</strong><span>{fmtTime(m.startsAt)} · {m.partySize} סועדים</span></div><div className="mine-actions"><select value={m.partySize} onChange={e=>editMine(m.id,Number(e.target.value))}>{Array.from({length:10},(_,i)=><option key={i+1}>{i+1}</option>)}</select><button type="button" onClick={()=>cancelMine(m.id)}>ביטול</button></div></div>):<p className="empty-copy">אין לכם הזמנות פעילות.</p>}</aside>}
