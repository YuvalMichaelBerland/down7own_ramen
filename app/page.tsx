...
export default function Home(){
  ...
  return <main>
    ...
    <section className="booking-shell" aria-labelledby="booking-title">
      <div className="date-card"><p className="kicker">הארוחה הבאה</p>{slots.length?<><div className="date-row"><div><strong>{new Date(slots[0].startsAt).getDate()}</strong><span>{new Date(slots[0].startsAt).toLocaleString('he-IL',{month:'short'})}</span></div><div className="date-copy"><h2 id="booking-title">{fmtDate(slots[0].startsAt)}</h2><p>{fmtTime(slots[0].startsAt)}—{fmtTime(slots[slots.length-1].endsAt)} · דאון טאון</p></div></div><div className="details"><span>משך הישיבה לפי המועד</span><span>מספר מקומות מוגבל</span></div></>:<><div className="empty-date">次</div><h2 id="booking-title">מועדים חדשים בקרוב</h2><p>עקבו אחרינו באינסטגרם לעדכון על הארוחה הבאה.</p></>}</div>
