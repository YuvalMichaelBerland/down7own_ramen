...
type Preference='none'|'chicken'|'vegetarian';
type MyReservation={id:string;startsAt:string;endsAt:string;partySize:number;preference:Preference;notes:string};
const PREFERENCE_LABELS:Record<Preference,string>={none:'ללא העדפה',chicken:'עוף',vegetarian:'צמחוני'};
...
export default function Home(){
  const { token, signIn, fetchAuthed } = useSession();
  const [slots,setSlots]=useState<Slot[]>([]),[activeDate,setActiveDate]=useState(''),[selected,setSelected]=useState<string>(),[partySize,setPartySize]=useState(1),[preference,setPreference]=useState<Preference>('none'),[notes,setNotes]=useState(''),[loading,setLoading]=useState(true),[error,setError]=useState(''),[confirmation,setConfirmation]=useState<Confirmation>();
  ...
  return <main>
    ...
    {token&&<aside className="mine-panel"><p className="kicker">ההזמנות שלי</p>{mine.length?mine.map(m=><div key={m.id} className="mine-row"><div><strong>{fmtDate(m.startsAt)}</strong><span>{fmtTime(m.startsAt)} · {m.partySize} סועדים</span></div><div className="mine-actions"><select value={m.partySize} onChange={e=>editMine(m.id,{partySize:Number(e.target.value)})}>{Array.from({length:10},(_,i)=><option key={i+1}>{i+1}</option>)}</select><button type="button" onClick={()=>cancelMine(m.id)}>ביטול</button></div><label className="mine-field">העדפה<select value={m.preference} onChange={e=>editMine(m.id,{preference:e.target.value as Preference})}>{Object.entries(PREFERENCE_LABELS).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label><label className="mine-field">הערה לשף<textarea defaultValue={m.notes} onBlur={e=>e.target.value!==m.notes&&editMine(m.id,{notes:e.target.value})}/></label></div>):<p className="empty-copy">אין לכם הזמנות פעילות.</p>}</aside>}
    ...
    <section className="booking-shell" aria-labelledby="booking-title">
      ...
      <div className="slot-panel"><div className="slot-heading"><div><p className="kicker">בחרו שעה</p><h3>{loading?'טוען…':slots.length?'מקומות פנויים':'אין מועדים פתוחים'}</h3></div><span className="live"><i/> בזמן אמת</span></div>
        ...
        {selectedSlot&&<div className="reserve-flow">
          ...
          <label>העדפת מנה <select value={preference} onChange={e=>setPreference(e.target.value as Preference)}>{Object.entries(PREFERENCE_LABELS).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label>
