...
type ReservationRow={id:string;guestName:string;guestEmail:string;partySize:number;preference:string;notes:string};
...
export default function Admin(){
  ...
  async function updateReservation(id:string,patch:Partial<Pick<ReservationRow,'guestName'|'guestEmail'|'partySize'|'preference'|'notes'>>){const r=await fetch('/api/admin/reservations',{method:'PATCH',headers:{'content-type':'application/json',...authHeaders()},body:JSON.stringify({id,...patch})});const d=await r.json() as {error?:string};if(!r.ok){setMessage(d.error||'לא הצלחנו לעדכן את ההזמנה');return;}await loadPublished();}
  ...
  return <main className="admin-page">
    ...
    <section className="admin-wrap">
      ...
      <div className="admin-column">
        ...
        {authorized&&<div className="admin-card reservations-card"><p className="kicker">הזמנות לפי תאריך</p><h2>אורחים</h2>{[...published,...history].length?[...published,...history].sort((a,b)=>b.startsAt.localeCompare(a.startsAt)).map(day=><div className="reservations-day" key={day.dayKey}><strong>{fmtDate(day.startsAt)}</strong>{day.reservations.length?<table className="reservations-table"><thead><tr><th>מזהה</th><th>אימייל</th><th>סועדים</th><th>העדפה</th><th>הערה</th><th/></tr></thead><tbody>{day.reservations.map(r=><Fragment key={r.id}>
          <tr><td>{r.id.slice(0,8).toUpperCase()}</td><td><input defaultValue={r.guestEmail} onBlur={e=>e.target.value!==r.guestEmail&&updateReservation(r.id,{guestEmail:e.target.value})}/></td><td><select defaultValue={r.partySize} onChange={e=>updateReservation(r.id,{partySize:Number(e.target.value)})}>{Array.from({length:10},(_,i)=><option key={i+1}>{i+1}</option>)}</select></td><td><select defaultValue={r.preference} onChange={e=>updateReservation(r.id,{preference:e.target.value})}><option value="none">ללא</option>{menuOptions.map(o=><option key={o.id} value={o.id}>{o.label}</option>)}</select></td><td><input defaultValue={r.notes} placeholder="—" onBlur={e=>e.target.value!==r.notes&&updateReservation(r.id,{notes:e.target.value})}/></td><td><button type="button" onClick={()=>setChatOpen(chatOpen===r.id?undefined:r.id)}>צ׳אט</button> <button type="button" onClick={()=>cancelReservation(r.id)}>ביטול</button></td></tr>
          {chatOpen===r.id&&<tr><td colSpan={6}><ChatPanel url={`/api/admin/reservations/${r.id}/messages`} authHeaders={authHeaders} self="admin"/></td></tr>}
        </Fragment>)}</tbody></table>:<p className="empty-copy">אין הזמנות ליום זה.</p>}</div>):<p className="empty-copy">אין עדיין הזמנות.</p>}</div>}
