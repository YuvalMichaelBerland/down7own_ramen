...
type PublishedSlot={id:string;startsAt:string;endsAt:string;capacity:number;reserved:number};
...
export default function Admin(){
  ...
  const [published,setPublished]=useState<PublishedSlot[]>([]);
  ...
  const loadPublished=useCallback(async()=>{try{const r=await fetch('/api/admin/slots',{headers:authHeaders()});const d=await r.json() as {isChef:boolean;slots?:PublishedSlot[]};setPlatformAdmin(d.isChef);setPublished(d.slots||[]);}catch{setPlatformAdmin(false);}},[authHeaders]);
  ...
  async function deleteSlot(slot:PublishedSlot){if(!window.confirm(`למחוק את ${fmtDate(slot.startsAt)} בשעה ${fmtTime(slot.startsAt)}?`))return;const r=await fetch('/api/admin/slots',{method:'DELETE',headers:{'content-type':'application/json',...authHeaders()},body:JSON.stringify({id:slot.id})});const d=await r.json() as {error?:string};setMessage(r.ok?'המשבצת נמחקה.':d.error||'לא הצלחנו למחוק את המשבצת');if(r.ok)await loadPublished();}
  ...
  return <main className="admin-page">
    ...
    <section className="admin-wrap">
      <div className="admin-heading"><p className="eyebrow">הדלפק של השף</p><h1>פתיחת זמני הזמנה</h1><p>צרו סדרת שעות במהירות, ואז התאימו כל משבצת בנפרד — שעה, משך ישיבה ומספר אורחים.</p></div>
      <div className="admin-column">
        ...
        {authorized&&<div className="admin-card published-card"><p className="kicker">משבצות שפורסמו</p><h2>מועדים פתוחים</h2>{published.length?<div className="published-list">{published.map(slot=><div className="published-slot" key={slot.id}><div><strong>{fmtDate(slot.startsAt)}</strong><span>{fmtTime(slot.startsAt)}–{fmtTime(slot.endsAt)} · {slot.reserved}/{slot.capacity} מוזמנים</span></div><button type="button" onClick={()=>deleteSlot(slot)}>מחיקה</button></div>)}</div>:<p className="empty-copy">אין כרגע מועדים פתוחים.</p>}</div>}
