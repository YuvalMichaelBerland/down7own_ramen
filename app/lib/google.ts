...
export async function verifyGoogleToken(token: string): Promise<GoogleUser> {
  ...
  const { payload } = await jwtVerify(token, googleKeys, { audience: process.env.GOOGLE_CLIENT_ID, issuer: ['https://accounts.google.com', 'accounts.google.com'] });
...
export function isChef(user: GoogleUser) { return Boolean(process.env.CHEF_EMAIL && user.email.toLowerCase() === process.env.CHEF_EMAIL.toLowerCase()); }
